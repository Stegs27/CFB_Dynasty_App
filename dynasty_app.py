...
def _standardize_recruiting_snapshot_df(df):
    ...
    if 'BlueChipRatio' not in df.columns:
        fs = pd.to_numeric(df.get('FiveStar', 0), errors='coerce').fillna(0)
        f4 = pd.to_numeric(df.get('FourStar', 0), errors='coerce').fillna(0)
        tc = pd.to_numeric(df.get('TotalCommits', 0), errors='coerce').fillna(0).replace(0, 1)
        df['BlueChipRatio'] = ((fs + f4) / tc).round(3)
    else:
        df['BlueChipRatio'] = pd.to_numeric(df['BlueChipRatio'], errors='coerce').fillna(0).round(3)
    df['Logo'] = df['Team'].apply(get_logo_source) if 'Team' in df.columns else ''
...
def _load_combined_recruiting_snapshot(class_type, year=None):
    class_type = str(class_type).strip().upper()
    for filename in ['recruiting_class_history_all.csv', 'recruiting_class_history.csv']:
        df = _load_recruiting_csv(filename)
        if df.empty or 'ClassType' not in df.columns:
            continue
        tmp = df.copy()
        tmp['ClassType'] = tmp['ClassType'].astype(str).str.strip().str.upper()
        tmp = tmp[tmp['ClassType'] == class_type].copy()
        if tmp.empty:
            continue
        if 'Year' in tmp.columns and year is not None:
            tmp = tmp[pd.to_numeric(tmp['Year'], errors='coerce') == pd.to_numeric(year, errors='coerce')].copy()
        elif 'Year' in tmp.columns and pd.to_numeric(tmp['Year'], errors='coerce').notna().any():
            max_year = pd.to_numeric(tmp['Year'], errors='coerce').dropna().max()
            tmp = tmp[pd.to_numeric(tmp['Year'], errors='coerce') == max_year].copy()
        if not tmp.empty:
            return _standardize_recruiting_snapshot_df(tmp)
    return pd.DataFrame()


def get_hs_recruiting_snapshot(year=None):
    df = _load_combined_recruiting_snapshot('HS', year=year)
    if not df.empty:
        return df
    df = _load_recruiting_csv('recruiting_high_school_history.csv')
    if not df.empty and 'Year' in df.columns and pd.to_numeric(df['Year'], errors='coerce').notna().any():
        yr = int(year) if year is not None else int(pd.to_numeric(df['Year'], errors='coerce').dropna().max())
        df = df[pd.to_numeric(df['Year'], errors='coerce') == yr].copy()
    return _standardize_recruiting_snapshot_df(df)


def get_portal_recruiting_snapshot(year=None):
    df = _load_combined_recruiting_snapshot('TRANSFER', year=year)
    if not df.empty:
        return df
...
def get_overall_recruiting_snapshot(year=None):
    df = _load_combined_recruiting_snapshot('OVERALL', year=year)
    if not df.empty:
        return df
    ...
    if not df.empty and 'Year' in df.columns and pd.to_numeric(df['Year'], errors='coerce').notna().any():
        yr = int(year) if year is not None else int(pd.to_numeric(df['Year'], errors='coerce').dropna().max())
        df = df[pd.to_numeric(df['Year'], errors='coerce') == yr].copy()
    df = _standardize_recruiting_snapshot_df(df)
    if not df.empty:
        return df
    return get_hs_recruiting_snapshot(year)
            ...
            try:
                ...
                for _, _cg in _cfp_res.iloc[::-1].iterrows():
                    ...
                    if _c_t1 in _user_teams_list or _c_t2 in _user_teams_list:
                        ...
                        _round_lower = str(_round).lower()
                        _is_title_game = any(k in _round_lower for k in ['national title', 'championship', 'title game', 'final'])
                        if not _is_title_game:
                            try:
                                _champs = pd.read_csv('champs.csv')
                                _champ_year_col = next((c for c in _champs.columns if c.strip().lower() == 'year'), None)
                                _champ_team_col = next((c for c in _champs.columns if c.strip().lower() in ['team', 'champion', 'winner', 'school']), None)
                                if _champ_year_col and _champ_team_col:
                                    _champ_row = _champs[pd.to_numeric(_champs[_champ_year_col], errors='coerce') == pd.to_numeric(CURRENT_YEAR, errors='coerce')]
                                    if not _champ_row.empty:
                                        _champ_name = str(_champ_row.iloc[-1][_champ_team_col]).strip().lower()
                                        if _champ_name and _champ_name == _winner.lower():
                                            _is_title_game = True
                            except Exception:
                                pass
                        if _is_title_game:
