...
st.title("🏈 Island Dynasty: Rivalry HQ")
...
def load_data():
    try:
        ...
        # 3. STATS & RIVALRY ENGINE
        stats_list = []
        h2h_rows = []
        rivalry_data = []

        # Find "Nail Biters" (Games <= 7 points)
        ...
        for i, user in enumerate(all_users):
            ...
            p_score = ((h_mov + v_mov)/2 * 5) + (n_wins * 25)

            stats_list.append({
                'User': user, 'Power Score': round(p_score, 1), 'Wins': wins, 'Losses': len(all_u_games)-wins,
                'Nattys': n_wins, 'Home Strength': round(h_mov, 1), 'Away Strength': round(v_mov, 1)
            })

            # H2H & Rivalry calculation
            h2h_row = {'User': user}
            for opp in all_users:
                if user == opp: 
                    ...
                    # Track close games for rivalry summary (only once per pair)
                    if i < all_users.index(opp):
                        ...
                        if close_count > 0:
                            rivalry_data.append({'Matchup': f"{user} vs {opp}", 'Close Games': close_count})
        ...
        return scores, rec, stats_df, pd.DataFrame(h2h_rows), pd.DataFrame(rivalry_data), all_users, years_available, col_meta
...
if data:
    ...
    with tabs[1]:
        ...
        st.subheader("The Rivalry Grid")
        st.table(h2h.set_index('User'))
        
        if not rivalries.empty:
            st.subheader("🔥 Most Intense Matchups")
            st.write("Matchups with the most games decided by 7 points or less:")
    ...
    with tabs[2]:
        ...
        st.success(f"🏆 Year {sel_year} Recap Active")
    ...
    with tabs[4]:
        ...
        # Keeping existing recruiting logic
        ...
        st.plotly_chart(px.line(rec_long.dropna(), x='Year', y='Rank', color=r_team_col).update_yaxes(autorange="reversed"))
