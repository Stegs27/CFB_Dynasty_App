...
with tabs[3]:
        ...
        # ── Last year recruiting class rank lookup ────────────────────────────
        _rec_rank_map = {}
        try:
            _rec_last = get_hs_recruiting_snapshot(year=CURRENT_YEAR - 1)
            if not _rec_last.empty and 'Team' in _rec_last.columns and 'Rank' in _rec_last.columns:
                _rec_rank_map = dict(zip(
                    _rec_last['Team'].astype(str).str.strip(),
                    pd.to_numeric(_rec_last['Rank'], errors='coerce')
                ))
        except Exception:
            pass

        # ── Poll movement: latest week vs previous week, same year ────────────
        _movement_map = {}
        try:
            _ph = pd.read_csv('cfp_rankings_history.csv')
            if not _ph.empty and 'YEAR' in _ph.columns and 'WEEK' in _ph.columns:
                _ph['RANK'] = pd.to_numeric(_ph['RANK'], errors='coerce')
                _ph['WEEK'] = pd.to_numeric(_ph['WEEK'], errors='coerce')
                _ly = _ph['YEAR'].max()
                _weeks = sorted(_ph.loc[_ph['YEAR'] == _ly, 'WEEK'].dropna().unique())
                if len(_weeks) >= 2:
                    _wk_now  = _weeks[-1]
                    _wk_prev = _weeks[-2]
                    _now  = _ph[(_ph['YEAR'] == _ly) & (_ph['WEEK'] == _wk_now)][['TEAM','RANK']].copy()
                    _prev = _ph[(_ph['YEAR'] == _ly) & (_ph['WEEK'] == _wk_prev)][['TEAM','RANK']].copy()
                    _merged_mv = _now.merge(_prev, on='TEAM', suffixes=('_now','_prev'))
                    for _, _mr in _merged_mv.iterrows():
                        _delta = float(_mr['RANK_prev']) - float(_mr['RANK_now'])  # positive = moved up
                        _movement_map[str(_mr['TEAM']).strip()] = _delta
        except Exception:
            pass
        ...
        for _, row in projected_field_display.sort_values('Projected Seed Display', ascending=True).head(12).iterrows():
            ...
            # Recruiting class rank (last year)
            _rec_rank = _rec_rank_map.get(team.strip())
            if pd.notna(_rec_rank) if _rec_rank is not None else False:
                _rec_disp = f"#{int(_rec_rank)}"
                _rec_color = '#4ade80' if _rec_rank <= 10 else ('#fbbf24' if _rec_rank <= 25 else '#94a3b8')
            else:
                _rec_disp  = '—'
                _rec_color = '#475569'
